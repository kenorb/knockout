...
FARPROC _scit_dbghelp_dll_ImageDirectoryEntryToData = 0;
FARPROC _scit_dbghelp_dll_ImageRvaToVa = 0;
...
int scitUnhookExports_EAT(LPCSTR lpDllName) {
	...
	aEATDiskMap		= _scitBuildEATMap(fileMap, FALSE);
...
//			memset(buffer2, 0, sizeof(buffer2));
//
//			itoa((int) *funcPointerDisk.pExportAddress, buffer1, 16);
//			itoa((int) *funcPointerMemory.pExportAddress, buffer2, 16);
//
//			MessageBoxA(0, buffer1, buffer2, 0);

			if (*funcPointerDisk.pExportAddress != *funcPointerMemory.pExportAddress) {
				MessageBoxA(0, funcPointerDisk.pFunctionName, funcPointerMemory.pFunctionName, 0);
			}

//			MessageBoxA(0, funcPointerMemory.pFunctionName, funcPointerMemory.pFunctionName, 0);
//			printf("%.8d 0x%.8X %s\n",
//				(int) funcPointerMemory.dwOrdinal,
//				(int) ((DWORD)hModule + (int) *funcPointerMemory.pExportAddress),
//				funcPointerMemory.pFunctionName
//			);
...
MessageBoxA(0, "ok4", lpModuleFilename, 0);
return 123;
